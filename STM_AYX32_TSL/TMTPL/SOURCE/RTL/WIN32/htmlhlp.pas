...
type
  ...
  HH_POPUP = packed record
    ...
    idString: Longint;        // string resource id, or text id if pszFile is specified in HtmlHelp call
...
//
// Base HTML Help functions
//
function HtmlHelpA conv arg_stdcall (hwndCaller: HWND; pszFile: PChar; uCommand: Longint; dwData: DWORD): HWND;
  external hhctrlocx name 'HtmlHelpA';

function HtmlHelpW conv arg_stdcall (hwndCaller: HWND; pszFile: PWideChar; uCommand: Longint; dwData: DWORD): HWND;
  external hhctrlocx name 'HtmlHelpW';

function HtmlHelp conv arg_stdcall (hwndCaller: HWND; pszFile: PChar; uCommand: Longint; dwData: DWORD): HWND;
  external hhctrlocx name 'HtmlHelpA';

//
// Additional HTML Help functions
//
function HtmlHelpShowHelp(HelpFile: String): HWND;
function HtmlHelpShowTopic(HelpFile: String; const aTopic: String): HWND;
function HtmlHelpShowContents(HelpFile: String): HWND;
function HtmlHelpShowIndex(HelpFile: String): HWND;
...
begin
  StrPCopy(pszFile, HelpFile);
  Result := HtmlHelp(0, pszFile, HH_DISPLAY_TOPIC, 0);
end;
...
begin
  StrPCopy(pszFile, HelpFile);
  Result := HtmlHelp(0, pszFile, HH_DISPLAY_TOC, 0);
end;
...
var
  pszFile: array [0..MAX_PATH] of Char;
begin
  Result := HtmlHelp(0, pszFile, HH_DISPLAY_INDEX, 0);
end;
